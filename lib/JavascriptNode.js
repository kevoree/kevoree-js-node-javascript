...
var JavascriptNode = AbstractNode.extend({
  ...
  updateLogLevel: function () {
    ...
    switch (logLevel.toLowerCase().trim()) {
      case 'all':
        if (this.log.level !== KevoreeLogger.ALL) {
          this.log.setLevel(KevoreeLogger.ALL);
        }
        break;

      case 'debug':
        if (this.log.level !== KevoreeLogger.DEBUG) {
      ...
      default:
      case 'info':
        if (this.log.level !== KevoreeLogger.INFO) {
          this.log.setLevel(KevoreeLogger.INFO);
        }
        break;

      case 'error':
        if (this.log.level !== KevoreeLogger.ERROR) {
          this.log.setLevel(KevoreeLogger.ERROR);
        }
        break;

      case 'warn':
        if (this.log.level !== KevoreeLogger.WARN) {
          this.log.setLevel(KevoreeLogger.WARN);
        }
        break;

      case 'quiet':
        if (this.log.level !== KevoreeLogger.QUIET) {
          this.log.setLevel(KevoreeLogger.QUIET);
        }
        break;
    }
  }
