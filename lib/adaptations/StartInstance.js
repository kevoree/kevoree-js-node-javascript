...
var StartInstance = AdaptationPrimitive.extend({
    ...
    if (this.modelElement.host && this.modelElement.host.name === this.node.getName()) {
      ...
      if (instance !== undefined && instance !== null) {
        ...
        if (!instance.isStarted()) {
          instance.__start__(timesUp('start(...)', 30000, function (err) {
            if (err) {
              this.log.error(this.toString(), 'Unable to start ' + instance.getPath());
            } else {
  ...
  undo: function(callback) {
    var cmd = new require('./StopInstance')(this.node, this.mapper, this.adaptModel, this.modelElement);
