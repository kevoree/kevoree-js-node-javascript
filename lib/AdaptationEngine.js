...
var AdaptationEngine = Class({
  ...
  processTrace: function(trace, modelElement) {
    var cmds = [],
      currentModel, instance, du, meta, currentModelElement, targetModelElement;
    ...
    if (!this.isVirtual(modelElement)) {
      switch (trace.refName) {
        ...
        case 'typeDefinition':
          switch (trace.traceType.name()) {
            case 'ADD':
              // console.log('>>>>> ADD TypeDef', modelElement.path());
              // console.log('\t', trace.srcPath);
              // console.log('\t', trace.previousPath);
              currentModel = this.node.getKevoreeCore().getCurrentModel();
              ...
              if (this.isRelatedToPlatform(currentModelElement)) {
                if (currentModelElement.path() === this.node.getPath()) {
                  // TODO if you arrive here, it means that you want to change this node platform TypeDefinition => What do we do then?
                } else {
                  meta = modelElement.select('deployUnits[name=*]/filters[name=platform,value=javascript]');
                  if (meta.size() > 0) {
                    du = meta.get(0).eContainer();
                    if (!this.modelObjMapper.getObject(du.path())) {
                      cmds.push(this.createCommand(AddDeployUnit, du));
                    ...
                    var error = new Error("no DeployUnit found for '" + modelElement.name + ": " + modelElement.typeDefinition.name + "/" + modelElement.typeDefinition.version + "' that matches the 'javascript' platform");
